...
const Checkout = () => {
  ...
  // Backend only supports Cash on Delivery (cod) payment method
  ...
  // Calculate order totals - must match backend calculations exactly
  const subtotal = cart?.items?.reduce(
    (sum, item) => sum + item.price * item.quantity,
    0
  ) || 0;
  // Backend uses $10 shipping for orders under $100
  const shipping = subtotal > 100 ? 0 : 10;
  // Backend uses 15% tax rate
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Ensure all price calculations match backend precision exactly
      const calculatedItemsPrice = Number(subtotal.toFixed(2));
      ...
      const calculatedTotalPrice = Number(
        (calculatedItemsPrice + calculatedTaxPrice + calculatedShippingPrice).toFixed(2)
      );
              ...
              {currentStep === 2 && (
                <div className="rounded-lg border bg-white p-6 shadow-sm">
                  ...
                  <form onSubmit={handlePaymentSubmit}>
                    <p className="text-sm text-muted-foreground mb-4">Currently, only Cash on Delivery is supported as a payment method.</p>
                    <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod} className="space-y-4">
                      {/* Credit Card Option - Disabled */}
                      <div className={`rounded-md border p-4 ${paymentMethod === 'credit-card' ? 'border-brand-blue bg-blue-50' : ''} opacity-50`}>
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem id="credit-card" value="credit-card" disabled />
                          <Label htmlFor="credit-card" className="flex items-center cursor-not-allowed">
                            <CreditCardIcon className="mr-2 h-5 w-5" />
                            Credit / Debit Card
                            <span className="ml-2 text-xs text-red-500">(Coming Soon)</span>
                          </Label>
                        </div>
                      ...
                      <div className={`rounded-md border p-4 ${paymentMethod === 'paypal' ? 'border-brand-blue bg-blue-50' : ''} opacity-50`}>
                        <div className="flex items-center space-x-2">
                      ...
                      {/* Cash on Delivery Option - Only enabled option */}
                      <div className="rounded-md border p-4 border-brand-blue bg-blue-50">
                        <div className="flex items-center space-x-2">
